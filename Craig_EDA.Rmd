...
df$avgAnnCount_log <- log(df$avgAnnCount) #log
...
```{r}
scatterplotMatrix( data=df, ~log(avgAnnCt_rate) +PercentMarried +  PctMarriedHouseholds , diagonal=c("histogram"))
```
## Marital Status by log incidence rate
```{r}
scatterplotMatrix( data=df, ~log(avgAnnCt_rate_log) +PercentMarried +  PctMarriedHouseholds , diagonal=c("histogram"))
```

## Marital Status by death / incidence ratio
```{r}
scatterplotMatrix( data=df, ~log(death_avgAnnCt_ratio) +PercentMarried +  PctMarriedHouseholds , diagonal=c("histogram"))
```
...
```{r}
scatterplotMatrix( data=df, ~avgAnnCount_log + PctPrivateCoverage + PctEmpPrivCoverage + PctPublicCoverage,diagonal=c("histogram") )
...
```{r}
cor(df[, c("avgAnnCount_log", "PctEmployed16_Over",  "PctUnemployed16_Over")], use = "complete.obs")
```
...
```
```{r}
hist(df$death_avgAnnCt_ratio, main= 'death/Incidence Ratio)',breaks =100, ylab = 'number of counties', xlab = 'Cancer Rate', labels = FALSE,  ,col = 'gray')
```
