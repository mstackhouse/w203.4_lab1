...
**PctSomeCol18_24:**
The variable for percentage of residents with some college education, aged 18-24, had `r sum(is.na(df$PctSomeCol18_24))` missing values (NA).  This represents a very large portion of the overall dataset - `r round(100*(sum(is.na(df$PctSomeCol18_24)))/(nrow(df)),3)`% of the variable's rows.  Due to the large number of missing values, this variable was not considered a key variable of the dataset.

**Median Age: **
There are 30 observations (`r round(100*(sum(is.na(df$MedianAge[df$MedianAge>300]))/(nrow(df))),5)`%`with median age>300. A median age of a county over 300 years of age is impossible in real life and these observations were removed.  This represents a small portion of the dataset. 
...
Median income and binned income are included 
The binned income 
...
|	medIncome		| `rPctBachDeg25_Overcor(df$deathRate, df$medIncome,  use = 'complete.obs')	`|
